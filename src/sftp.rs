...
impl Drop for File {
	fn drop(&mut self) {
		unsafe {
			let close_handle = raw::libssh2_sftp_close_handle(self.raw);
			if close_handle != 0 {
				eprintln!(
					"SSH2:warning: Dropping file {:?} led to status code {}. (last sftp error: {})",
					self.raw,
					close_handle,
					raw::libssh2_sftp_last_error(self.sftp.raw)
				)
			}
		}
	}
